...
RUN dotnet build "MarkdownLinkCheckLogParserCli.csproj" -c Release -o /app/build --no-restore

FROM build AS publish
# use ''--no-build' and ''p:OutDir' to pickup the output from dotnet build and avoid building again
RUN dotnet publish "MarkdownLinkCheckLogParserCli.csproj" -c Release -p:OutDir=/app/build -o /app/publish --no-build
